...
export class PushNotificationService {
  ...
  static async sendToAll(notification) {
    ...
    for (const doc of subDocs) {
      ...
      // Attempt to find a user linked to this subscription (if subscription doc stored endpoint)
      let user = null
      try {
        if (doc.subscriptionId || doc.endpoint) {
          const endpointKey = doc.subscriptionId || Buffer.from(doc.endpoint || '').toString('base64').slice(0,50)
          user = await db.collection('users').findOne({ 'pushSubscriptionId': endpointKey })
        }
      } catch (e) {
        // ignore
      }
