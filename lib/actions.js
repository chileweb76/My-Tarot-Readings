...
export async function getTagsAction() {
  try {
    ...
    const tags = await db.collection('tags')
      .find({ $or: [{ userId: user._id }, { userId: null, isGlobal: true }] })
      .toArray()
...
export async function updateReadingAction(formData) {
  try {
    const user = await getUserFromToken()
    const readingId = formData.get('readingId')
    const readingData = JSON.parse(formData.get('readingData'))
    ...
    const result = await db.collection('readings').findOneAndUpdate(
      { _id: new ObjectId(readingId), userId: user._id },
      { $set: updateData },
    ...
    return { error: 'Failed to update reading' }
  }
}
