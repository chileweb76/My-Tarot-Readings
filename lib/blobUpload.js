...
export async function uploadImageToBlob(readingId, imageFile) {
  try {
    const _DEBUG = typeof process !== 'undefined' && (process.env.NODE_ENV !== 'production' || process.env.NEXT_PUBLIC_DEBUG === 'true')
    if (_DEBUG) console.log('🔵 uploadImageToBlob: Starting client-side upload', {
      readingId,
  ...
  if (_DEBUG) console.log('� uploadImageToBlob: Using FINAL endpoint with ultra cache-busting:', finalUrl)
    
    // Skip CORS test and go straight to the final endpoint
  if (_DEBUG) console.log('� uploadImageToBlob: Skipping tests, going straight to FINAL endpoint')
    
    // Use the FINAL endpoint with completely different naming
    const url = finalUrl
    
    if (_DEBUG) console.log('� uploadImageToBlob: FINAL Configuration', {
    ...
    if (_DEBUG) console.log('� uploadImageToBlob: Making FINAL request', {
      url,
    ...
    if (_DEBUG) console.log('🔵 uploadImageToBlob: Response received', {
    ...
    if (!response.ok) {
      ...
      if (_DEBUG) console.error('🔴 Client blob upload failed:', {
        status: response.status,
        statusText: response.statusText,
        errorData,
        url
    ...
    if (_DEBUG) console.log('🟢 uploadImageToBlob: Upload successful:', { 
  ...
  if (_DEBUG) console.error('🔴 uploadImageToBlob error:', error)
    return { 
      success: false,
...
const blobUploadModule = { uploadImageToBlob, isBlobUploadReady }
export default blobUploadModule