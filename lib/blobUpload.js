...
// These functions run in the br    console.log('🚀 uploadImageToBlob: Making FINAL request', {
      url,
      readingId,
      headers: { ...headers, Authorization: headers.Authorization ? 'Bearer [REDACTED]' : undefined },
      hasFormData: finalFormData instanceof FormData,
      formDataKeys: Array.from(finalFormData.keys()),
      formDataEntries: Array.from(finalFormData.entries()).map(([key, value]) => ({
        key, 
    ...
    }) avoiding Server Action conflicts
...
export async function uploadImageToBlob(readingId, imageFile) {
  try {
    ...
    const finalUrl = `${apiBaseUrl}/api/upload-image?readingId=${readingId}&t=${cacheBuster}&rand=${Math.random()}`
    
    console.log('🚀 uploadImageToBlob: Using UPLOAD-IMAGE endpoint (brand new name):', finalUrl)
    console.log('🚀 uploadImageToBlob: File details:', {
      name: imageFile.name,
      size: imageFile.size,
      type: imageFile.type,
    ...
    // Skip CORS test and go straight to the upload endpoint
    console.log('🚀 uploadImageToBlob: Direct upload to new endpoint')
