...
export async function uploadImageToBlob(readingId, imageFile) {
  try {
    ...
    // First try a simple CORS test to see if the issue is specific to the blob endpoint
    const testUrl = `${apiBaseUrl}/api/cors-test-simple?readingId=${readingId}&t=${cacheBuster}`
    
    console.log('🔵 uploadImageToBlob: Testing CORS with simple endpoint first:', testUrl)
    ...
    try {
      ...
      console.log('🔵 uploadImageToBlob: CORS test result:', {
        status: testResponse.status,
        ok: testResponse.ok
      })
      
      if (!testResponse.ok) {
        console.warn('🟡 uploadImageToBlob: Simple CORS test failed, trying V2 anyway')
      }
    } catch (testError) {
      console.error('🔴 uploadImageToBlob: CORS test failed:', testError)
    }
    
    // Try alternative endpoint with completely different name
    const url = `${apiBaseUrl}/api/upload-alternative?readingId=${readingId}&t=${cacheBuster}`
    
    console.log('🔵 uploadImageToBlob: Using alternative endpoint to bypass all caching:', url)
    ...
    let headers = {
      ...
      'x-reading-id': readingId // Pass reading ID as header for V2 endpoint
    }
