...
export async function uploadImageToBlob(readingId, imageFile) {
  try {
    ...
    const finalUrl = `${apiBaseUrl}/api/image-upload-final?readingId=${readingId}&t=${cacheBuster}&rand=${Math.random()}`
    ...
    const response = await fetch(url, {
      ...
      credentials: 'omit', // Don't send credentials to match wildcard CORS
