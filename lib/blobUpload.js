...
export async function uploadImageToBlob(readingId, imageFile) {
  try {
    ...
    // Simple cache-busting to avoid URL length issues
    const cacheBuster = Date.now()
    
    // Try the upload-image endpoint with clean URL
    const finalUrl = `${apiBaseUrl}/api/upload-image?readingId=${readingId}&t=${cacheBuster}`
