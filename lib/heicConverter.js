...
export async function ensurePreviewableImage(file) {
  ...
  console.log('Processing file:', name, file.type, 'isHeic:', isHeic)
  
  if (!isHeic) {
    const url = URL.createObjectURL(file)
    console.log('Non-HEIC file, returning object URL:', url)
  ...
  if (typeof createImageBitmap === 'function' && typeof document !== 'undefined') {
    try {
      console.log('Attempting HEIC conversion using createImageBitmap...')
      ...
      console.log('HEIC conversion successful, returning converted file:', jpgName, url)
  ...
  const url = URL.createObjectURL(file)
  console.log('HEIC fallback, returning original file with object URL:', url)
  return { file, previewUrl: url }
}
