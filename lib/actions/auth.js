...
export async function resetPasswordAction(formData) {
  try {
    const email = formData.get('email')
    // This action requests a reset email (forgot) — not the token consumption
    const response = await makeAuthenticatedAPICall('/api/auth/forgot', {
      method: 'POST',
...
// Submit new password using token (consumes the reset token)
export async function submitNewPasswordAction(formData) {
  try {
    const token = formData.get('token')
    const newPassword = formData.get('newPassword')
    const verifyPassword = formData.get('verifyPassword')

    const apiBase = (process.env.NEXT_PUBLIC_API_URL || process.env.API_BASE_URL || 'http://localhost:5000').replace(/\/$/, '')
    const response = await fetch(`${apiBase}/api/auth/reset`, {
      method: 'POST',
    ...
    // Attempt to parse JSON, otherwise return plain text as error
    if (!response.ok) {
      try {
        const err = JSON.parse(text)
        throw new Error(err && err.error ? err.error : text || 'Reset failed')
      } catch (e) {
    ...
    try {
      ...
      return { success: true, message: text || 'Password reset successful' }
    }
  } catch (err) {
    console.error('submitNewPasswordAction error:', err)
