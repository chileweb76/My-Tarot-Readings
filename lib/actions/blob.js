...
export async function uploadBlobAction(readingId, imageFile) {
  try {
    ...
    // Get auth token and make direct fetch call (FormData doesn't work well with makeAuthenticatedAPICall)
    ...
    const response = await fetch(`${apiBaseUrl}/api/readings/${readingId}/blob/upload`, {
      method: 'POST',
      body: formData,
      headers: {
    ...
    console.error('🔴 uploadBlobAction error:', error)
    return { error: error.message || 'Failed to upload blob' }
  }
}
