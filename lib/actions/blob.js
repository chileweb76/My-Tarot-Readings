...
export async function uploadBlobAction(formData) {
  try {
    // Try to get user, but don't fail completely if token issues exist
    let user = null
    try {
      user = await getUserFromToken()
    } catch (tokenError) {
      console.warn('Token validation failed in uploadBlobAction, proceeding with anonymous upload:', tokenError.message)
    }
    ...
    // Use the backend blob upload endpoint specifically for readings
    const response = await makeAuthenticatedAPICall(`/api/blob-upload?id=${readingId}`, {
      method: 'POST',
      body: formData,
      headers: {
