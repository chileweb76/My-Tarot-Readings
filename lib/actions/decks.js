...
export async function getDecksAction() {
  try {
    ...
    try {
      ...
      authError = err
      // User not authenticated - will fetch global decks only
    }

    let response
    if (user) {
      // Authenticated: fetch all decks (global + user-owned) with auth
...
export async function getSingleDeckAction(deckId) {
  try {
    ...
    // Backend returns { success: true, deck: {...} }
    ...
    const deck = {
      ...deckData,
      _id: deckData._id?.toString ? deckData._id.toString() : deckData._id,
      createdAt: deckData.createdAt ? (new Date(deckData.createdAt)).toISOString() : undefined,
      updatedAt: deckData.updatedAt ? (new Date(deckData.updatedAt)).toISOString() : undefined
    }
...
export async function updateDeckAction(formData) {
  try {
    ...
    if (!response.ok) {
      const errorData = await response.text()
      throw new Error(errorData || 'Failed to update deck')
    }
    ...
    return { error: error.message || 'Failed to update deck' }
  }
}
...
/**
 * Update deck image URL in MongoDB after client-side blob upload
 */
export async function updateDeckImageUrlAction(deckId, imageUrl) {
  try {
    const user = await getUserFromToken()
    
    if (!deckId || !imageUrl) {
      return { success: false, error: 'Deck ID and image URL are required' }
    }

    const response = await makeAuthenticatedAPICall(`/api/decks/${deckId}/image`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    if (!response.ok) {
      const errorData = await response.text()
      throw new Error(errorData || 'Failed to update deck image')
    }
    ...
    return { success: false, error: error.message || 'Failed to update deck image' }
  }
}

/**
 * Update card image URL in MongoDB after client-side blob upload
 */
export async function updateCardImageUrlAction(deckId, cardName, imageUrl) {
  try {
    const user = await getUserFromToken()
    
    if (!deckId || !cardName || !imageUrl) {
      return { success: false, error: 'Deck ID, card name, and image URL are required' }
    }

    const response = await makeAuthenticatedAPICall(`/api/decks/${deckId}/card/${encodeURIComponent(cardName)}/image`, {
      method: 'PATCH',
      headers: {
        'Content-Type': 'application/json'
      },
    ...
    if (!response.ok) {
      const errorData = await response.text()
      throw new Error(errorData || 'Failed to update card image')
    }
    ...
    return { success: false, error: error.message || 'Failed to update card image' }
  }
}