...
export async function getDecksAction() {
  try {
    ...
    try {
      ...
      console.log('🟡 getDecksAction: Auth failed, trying unauthenticated call:', authError.message)
      // Fall back to direct API call without authentication
      const baseUrl = process.env.VERCEL_URL 
        ? `https://${process.env.VERCEL_URL}` 
        : (process.env.CLIENT_URL || 'http://localhost:3000')
      
      const url = `${baseUrl}/api/decks`
      console.log('🟡 getDecksAction: Using fallback URL:', url)
