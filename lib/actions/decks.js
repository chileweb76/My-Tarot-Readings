...
export async function getDecksAction() {
  try {
    ...
    // Always try authenticated call first to get both global + user decks
    let response
    let user = null
    
    try {
      user = await getUserFromToken()
      console.log('🔵 getDecksAction: User authenticated:', user ? 'YES' : 'NO')
      response = await makeAuthenticatedAPICall('/api/decks')
      console.log('🔵 getDecksAction: Using authenticated call - should return all decks')
    } catch (authError) {
      console.log('🟡 getDecksAction: Auth failed, falling back to global decks only:', authError.message)
      // Fall back to direct backend call without authentication (global decks only)
      ...
      console.log('🟡 getDecksAction: Using direct backend URL (global only):', url)
    ...
    console.log('🔵 getDecksAction: Raw backend response:', JSON.stringify(data, null, 2))
    const rawDecks = Array.isArray(data) ? data : (data.decks || [])
    console.log('🔵 getDecksAction: Raw decks array:', rawDecks.length, 'items')
    ...
    console.log('🟢 getDecksAction: Successfully processed', decks.length, 'decks:')
    decks.forEach((deck, i) => {
      console.log(`🟢   ${i+1}. "${deck.deckName}" (ID: ${deck._id}) ${deck.isGlobal ? '[GLOBAL]' : '[USER]'}`)
    })
