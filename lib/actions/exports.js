...
export async function exportReadingPDFAction(readingData) {
  try {
    ...
    try {
      ...
      if (header !== '%PDF-') {
        // Decode a small preview as text for diagnostics
        let preview = null
        try { preview = new TextDecoder().decode(bytes.subarray(0, 1024)) } catch (e) { preview = '<unable to decode preview>' }
        console.error('Export PDF: response did not contain a PDF. Preview:', preview)
