...
export async function getUserFromToken() {
  ...
  try {
    ...
    try {
      ...
      if (verifyError.message && (verifyError.message.includes('SubtleCrypto') || verifyError.message.includes('3rd argument'))) {
        console.warn('JWT verify failed due to crypto issue, falling back to decode:', verifyError.message)
        decoded = jwt.decode(token)
        if (!decoded) {
          throw new Error('Token decode also failed')
        }
        // Add a warning flag so we know this is unverified
        decoded._unverified = true
      } else {
        throw verifyError
      }
    }
...
export async function makeAuthenticatedAPICall(endpoint, options = {}) {
  ...
  // Ensure apiBaseUrl is defined and valid
  if (!apiBaseUrl || typeof apiBaseUrl !== 'string') {
    apiBaseUrl = 'https://mytarotreadingsserver.vercel.app'
    console.warn('⚠️ API Base URL not found in environment variables, using fallback:', apiBaseUrl)
  }
