...
export const getSpreadImageUrl = async (spreadName, options = {}) => {
  ...
  if (blobMapping && blobMapping.spreads) {
    const staticPath = `/images/spreads/${spreadName.toLowerCase().replace(/\s+/g, '-')}.jpg`
    const blobUrl = blobMapping.spreads[staticPath]
    if (blobUrl) {
      console.log(`🎯 Using blob URL for spread ${spreadName}:`, blobUrl)
      return blobUrl
    }
    
    // Try with .png extension
    const pngPath = `/images/spreads/${spreadName.toLowerCase().replace(/\s+/g, '-')}.png`
    const pngBlobUrl = blobMapping.spreads[pngPath]
    if (pngBlobUrl) {
      console.log(`🎯 Using blob URL for spread ${spreadName} (PNG):`, pngBlobUrl)
      return pngBlobUrl
    }
  }
  
  // Fallback to API call
  const cacheKey = `spread-${spreadName}`
  const cached = cache.get(cacheKey)
  ...
  try {
    const { apiFetch } = await import('./api')
    const params = new URLSearchParams({ spreadName, ...options })
    
    const response = await apiFetch(`/spreads/image?${params}`)
    if (!response.ok) {
      console.warn(`Failed to fetch spread image for ${spreadName}:`, response.status)
      return null
    }
