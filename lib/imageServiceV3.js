...
export const getSpreadImageUrl = async (spreadId, options = {}) => {
  ...
  // Use cache to avoid repeated server calls
  const cacheKey = `spread_${spreadId}_${JSON.stringify(options)}`
  const cached = cache.get(cacheKey)
  if (cached && (Date.now() - cached.timestamp) < CACHE_TTL) {
    return cached.data
  }
  ...
  if (blobMapping) {
    ...
    for (const key of uniqueCandidates) {
      if (spreads[key]) {
        const url = spreads[key]
        cache.set(cacheKey, { data: url, timestamp: Date.now() })
        return url
      }
    }
  }

  // Server fallback: attempt to fetch spread record from backend and derive image
  try {
    let apiBaseUrl = process.env.API_BASE_URL || process.env.NEXT_PUBLIC_API_URL || 'https://mytarotreadingsserver.vercel.app'
    if (!apiBaseUrl || typeof apiBaseUrl !== 'string') apiBaseUrl = 'https://mytarotreadingsserver.vercel.app'
    const url = `${apiBaseUrl}/api/spreads/${encodeURIComponent(spreadId)}`
    const res = await fetch(url, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
        'User-Agent': 'MyTarotReadings-ImageService/1.0'
      }
    })

    if (res.ok) {
      const data = await res.json()
      const imageField = data && (data.image || data?.data?.image || null)

      if (imageField && typeof imageField === 'string') {
        // If it's already a full URL, use it
        if (imageField.startsWith('http') || imageField.startsWith('blob:') || imageField.startsWith('data:')) {
          cache.set(cacheKey, { data: imageField, timestamp: Date.now() })
          return imageField
        }

        // If server returned a path/key, check blobMapping.spreads
        if (blobMapping && blobMapping.spreads && blobMapping.spreads[imageField]) {
          const mapped = blobMapping.spreads[imageField]
          cache.set(cacheKey, { data: mapped, timestamp: Date.now() })
          return mapped
        }

        // Last attempt: use transformImageUrl to resolve (will call Server Action fallback for spreads if implemented)
        const transformed = await transformImageUrl(imageField, IMAGE_TYPES.SPREAD, { spreadName: data.spread, spreadId: data._id })
        if (transformed) {
    ...
    // Server lookup failed — suppress noisy warning in production
  }
  // No blob mapping found for spread; returning null
  return null
}
...
export const transformImageUrl = async (originalUrl, imageType, context = {}) => {
  ...
  switch (imageType) {
    ...
    default:
      // Unknown image type — no transformation available
      return originalUrl
  }
}
